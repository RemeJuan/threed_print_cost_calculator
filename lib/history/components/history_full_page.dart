...
import 'package:threed_print_cost_calculator/history/components/history_empty_state.dart';
import 'package:threed_print_cost_calculator/history/components/history_list_view.dart';
import 'package:threed_print_cost_calculator/history/components/history_overflow_hint.dart';
import 'package:threed_print_cost_calculator/history/components/history_search_bar.dart';
import 'package:threed_print_cost_calculator/history/hooks/history_overflow_hint.dart';
import 'package:threed_print_cost_calculator/history/hooks/history_page_actions.dart';
import 'package:threed_print_cost_calculator/history/hooks/history_search_query.dart';
...
class HistoryFullPage extends HookConsumerWidget {
  const HistoryFullPage({
    required this.l10n,
    required this.paged,
    required this.policy,
    required this.appRefreshTick,
    required this.actions,
    required this.onHistoryLoaded,
    super.key,
  });

  final AppLocalizations l10n;
  final HistoryPagedState paged;
  final PremiumAccessPolicy policy;
  final int appRefreshTick;
  final HistoryPageActions actions;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    useEffect(() {
      void scrollListener() {
        ...
        if (current >= (max - threshold) && !hasReachedHistoryLimit) {
          final notifier = ref.read(historyPagedProvider.notifier);
