...
- Stack trace detail captured from Crashlytics samples: `io.flutter.embedding.engine.FlutterJNI.nativeSurfaceCreated` → `FlutterJNI.onSurfaceCreated` → `FlutterRenderer.*` → `surfaceCreated`
...
- Device distribution: the regression cluster was concentrated on a single device/OS combination in Crashlytics samples
...
- Manual flow check: announcement presentation stays suppressed when an overlay route is on top or when the route changes before the post-frame callback runs.
...
## Fix branch/worktree
- Branch: `fix/crashlytics-8aeb52bf4fad6628513cae2330651dd6-flutterjni-anr-regression-2`
