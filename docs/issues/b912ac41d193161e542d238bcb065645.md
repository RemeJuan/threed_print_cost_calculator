...
## Regression V2 (2026-05-19, v2.10.0+204)
- New SIGNAL_REGRESSED on May 19, 2026 in version 2.10.0 (204).
- Signal: SIGNAL_EARLY (100% crashes in first second of session).
- Signal: SIGNAL_FRESH (First appeared 7 days ago).
- This Kanban task (t_a6959001) assigned to investigate.

### Verification (2026-05-19)
Verified fix state in `main` branch at commit `e9913041` (v2.10.0+10):

1. **ndk.abiFilters** (lines 61-67): Explicitly set to `'arm64-v8a', 'armeabi-v7a'` in `defaultConfig`. ✅
2. **Splits block**: Fully commented out. ✅
3. **jniLibs useLegacyPackaging** (lines 98-102): `packagingOptions { jniLibs { useLegacyPackaging true } }` present. ✅

Both fixes (PRs #56 and #60) are present and unchanged since May 12.

### Assessment: Residual Signal
The regression signal is **residual** — users on older app versions (2.9.4 through 2.10.0+9) who haven't updated to 2.10.0+10 still encounter the crash. The fix shipped in 2.10.0+10 is correct and complete. No code changes are required.

**Root cause confirmed**: Flutter/Gradle/Play Store AAB pipeline edge case that `ndk.abiFilters + useLegacyPackaging` addresses. The configuration has been stable since b0b2c532 (May 14) with no changes to the Android build pipeline between PR #60 and v2.10.0 release.

**No action required** — status remains `released`.

### Known upstream tracking
- Primary Flutter tracker: https://github.com/flutter/flutter/issues/151638
- Related open issues in 3.41.x Flutter: #164426, #171317, #180192 — all show same pattern; no new mitigation available beyond current approach.
